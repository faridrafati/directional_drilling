  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-6">
      <div className="w-full max-w-[1700px] mx-auto flex flex-col flex-1 min-h-0">
        <div className="mb-4 shrink-0 flex items-start justify-between gap-4 flex-wrap">
          ...
          {user && (
            <div className="flex items-center gap-2 text-xs">
              ...
              <button onClick={() => setChanging(true)} className="h-8 px-3 rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors duration-150">Password</button>
              <button onClick={signOut} className="h-8 px-3 rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors duration-150">Sign out</button>
            </div>
          )}
        </div>
...
function ReportsWorkspace({ wells, isAdmin }: { wells: EntryWell[]; isAdmin: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!wells.some((w) => w.id === wellId)) { setWellId(wells[0].id); setReportId(null); }
  }, [wells, wellId]);
  ...
  return (
    <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 overflow-hidden">
      {/* Well + days */}
      <div className="flex flex-col min-h-0 bg-white border border-gray-200 rounded-lg shadow-sm p-3 overflow-y-auto">
        <label className="block">
          <span className="text-[11px] text-gray-500">Well</span>
          <select value={wellId} onChange={(e) => { setWellId(e.target.value); setReportId(null); }}
        ...
        <div className="mt-3 pt-3 border-t border-gray-100">
          <span className="text-[11px] text-gray-500">New day</span>
          <div className="flex gap-1.5 mt-1">
            <JalaliDatePicker value={newDate} onChange={setNewDate} placeholder="1404/05/09" className="flex-1 min-w-0" />
            <button onClick={createDay} disabled={!newDate || creating}
              className="h-9 px-3 text-sm rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors duration-150 disabled:bg-gray-300 shrink-0">
              {creating ? "…" : "Add"}
            </button>
          </div>
          <p className="text-[10px] text-gray-400 mt-1">Opens a draft with yesterday's midnight depth carried forward.</p>
        </div>

        {error && <div className="mt-2 text-[11px] text-red-700 bg-red-50 border border-red-200 rounded px-2 py-1">{error}</div>}

        <div className="mt-3 pt-3 border-t border-gray-100 flex-1 min-h-0">
          ...
          <div className="space-y-1">
            {(daysQ.data ?? []).map((r) => (
              <div key={r.id}
                className={`group w-full flex items-center gap-1 rounded-md border px-2 py-1 cursor-pointer transition-colors duration-100 ${
                  reportId === r.id ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-blue-50/50"}`}
                onClick={() => setReportId(r.id)}>
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] font-medium text-gray-800 tabular-nums">#{r.serialNo} · {r.reportDate}</div>
                  <div className="text-[10px] text-gray-400 truncate">
                    {r.midnightDepth != null ? `${r.midnightDepth} m` : "no depth yet"} · {r.user.username}
                  </div>
                </div>
                <span className={`text-[9px] px-1 py-0.5 rounded uppercase shrink-0 ${
                ...
                {(r.status === "draft" || isAdmin) && (
                  <button title="Delete this day" aria-label="Delete this day" onClick={(e) => { e.stopPropagation(); void removeDay(r.id); }}
                    className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-600 text-xs shrink-0 transition-colors duration-150">×</button>
                )}
              </div>
            ))}
            {daysQ.data?.length === 0 && (
              <p className="text-[11px] text-gray-400">No reports on this well yet — add the first day above.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="min-h-0 overflow-auto">
        ...
        {!reportId && (
          <div className="h-full flex items-center justify-center text-sm text-gray-400 border border-dashed border-gray-300 rounded-lg">
            Pick a day on the left, or add a new one.
          </div>
        )}
        {reportQ.data && (
          <ReportEditor
