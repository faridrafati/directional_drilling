...
uses
  ...
  Keyboard,
  GLScene, GLWin32Viewer, GLMisc, GLObjects, GLCadencer, vectorGeometry, GLGraphics,
  GLHUDObjects, GLTexture, OpenGL1x, GLTeapot, GLTexCombineShader, GLUserShader, AsyncTimer,
  GLCrossPlatform;
...
type
  TAaStereoForm = class(TForm)
    ...
    procedure SetupRender(Sender: TObject;var rci: TRenderContextInfo);
    procedure CleanupRender(Sender: TObject;var rci: TRenderContextInfo);
...
var flag:BYTEBOOL; flagged:PGLboolean;
begin
  ...
  begin
    Pinout1.Caption:='NA';
    StereoCapable :=False;
  end else StereoCapable :=True;

  StereoMode:=2;
  if (not WGL_ARB_pbuffer) then showmessage('no p buffer');
{   WGL_ARB_pbuffer:=CheckExtension('WGL_STEREO_ARB ');
  if (not WGL_DOUBLE_BUFFER_ARB) then showmessage('no double buffer');
  if (not WGL_STEREO_ARB) then showmessage('no stereo buffer');}
...
procedure TAaStereoForm.FormCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
...
procedure TAaStereoForm.GLCadencer1Progress(Sender: TObject; const deltaTime,
  newTime: Double);
begin
  If Rotate1.Checked then GLTeapot1.Turn(deltaTime*25 );
  ...
  2:Begin
    ...
    End;
  3:Begin {Need to Write to Left and Right Buffers}
...
procedure TAaStereoForm.SetupRender(Sender: TObject;var rci: TRenderContextInfo);
begin
  case RenderPass of
    1 : glColorMask(True,False,False,True);
    2 : glColorMask(False,True,False,True);
    3 : glColorMask(False,False,True,True);
  else
    glColorMask(True,True,True,True);
  end;
end;

procedure TAaStereoForm.CleanupRender(Sender: TObject;var rci: TRenderContextInfo);
begin
  glColorMask(True,True,True,True);
end;
...
begin
  ...
  Case Key of
    ...
    'h':begin
     ...
     'f':begin
           GLCamera1.FocalLength:=GLCamera1.FocalLength+10;// Camera focus plane foward */
           LeftCamera.FocalLength:=LeftCamera.FocalLength+10;
           RightCamera.FocalLength:=RightCamera.FocalLength+10;
         end;
     'v':begin
           GLCamera1.FocalLength:=GLCamera1.FocalLength-10;// Camera focus plane back */
           LeftCamera.FocalLength:=LeftCamera.FocalLength-10;
           RightCamera.FocalLength:=RightCamera.FocalLength-10;
         end;
     'd':begin
           GLCamera1.DepthOfView:=GLCamera1.DepthOfView+10;// Camera DepthOfView foward */
           LeftCamera.DepthOfView:=LeftCamera.DepthOfView+10;
           RightCamera.DepthOfView:=RightCamera.DepthOfView+10;
         end;
     'c':begin
           GLCamera1.DepthOfView:=GLCamera1.DepthOfView-10;// Camera DepthOfView back */
           LeftCamera.DepthOfView:=LeftCamera.DepthOfView-10;
           RightCamera.DepthOfView:=RightCamera.DepthOfView-10;
         end;
  end;//end keyboard
